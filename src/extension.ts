...
function replaceEditorSelection(locale: string) {
    const timeStamp: string = composeTimeStamp(new Date(), locale);
...
export function activate(context: vscode.ExtensionContext) {
    let disposable = [
        vscode.commands.registerCommand('inserttimestamp.perform', () => {  
            vscode.window.showQuickPick(ISOCountries)
            .then(replaceEditorSelection);                      
        })
    ];
